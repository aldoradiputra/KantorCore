...
export default async function SODetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const data = await getSO(ctx.tenant.id, id)
  if (!data) notFound()
    ...
    >
      <div style={{ padding: 'var(--s-6)', display: 'flex', flexDirection: 'column', gap: 'var(--s-5)', maxWidth: 900 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 'var(--s-4)' }}>
          <div>
            <div style={{ marginBottom: 4 }}>
              <Link href="/sales/orders" style={{ font: '13px/1 var(--font-sans)', color: 'var(--fg-3)', textDecoration: 'none' }}>← Penawaran & Penjualan</Link>
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--s-4)', padding: 'var(--s-4)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', background: 'var(--surface)' }}>
          <InfoRow label="Pelanggan" value={so.customerName} />
          {customerContact && <InfoRow label="Kontak" value={customerContact.name + (customerContact.email ? ` · ${customerContact.email}` : '')} />}
          <InfoRow label="Tanggal" value={so.date} />
          {so.expiryDate && <InfoRow label="Berlaku Hingga" value={so.expiryDate} />}
          {so.notes && <InfoRow label="Catatan" value={so.notes} />}
          {so.invoiceId && (
            <InfoRow label="Faktur" value={
              <Link href={`/fin/invoices/${so.invoiceId}`} style={{ color: 'var(--indigo)', textDecoration: 'none' }}>Lihat faktur →</Link>
            } />
          )}
        </div>
        ...
        <div style={{ border: '1px solid var(--border)', borderRadius: 'var(--r-md)', overflow: 'hidden', background: 'var(--surface)' }}>
          ...
          <div style={{ padding: '12px 14px', display: 'flex', justifyContent: 'flex-end', borderTop: '1px solid var(--border)', background: 'var(--bg)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 24, minWidth: 200 }}>
        ...
        <div>
          ...
          <Chatter entityType="sales.order" entityId={so.id} />
        </div>
      </div>
    </SalesShell>
  )
