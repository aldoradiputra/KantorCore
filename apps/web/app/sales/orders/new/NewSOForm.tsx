...
export function NewSOForm({ accounts, taxes, contacts, products }: {
  accounts: AccountOpt[]
  taxes: TaxOpt[]
  contacts: ContactOpt[]
  products: ProductOpt[]
  ...
  return (
    <div style={{ padding: 'var(--s-6)', maxWidth: 900 }}>
      <header style={{ marginBottom: 'var(--s-4)' }}>
        ...
        <p style={{ font: '13px/1.5 var(--font-sans)', color: 'var(--fg-3)', margin: '6px 0 0' }}>Penawaran tersimpan dalam status Draft. Konfirmasi untuk menjadi Sales Order.</p>
      </header>
      ...
      <form onSubmit={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--s-4)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--s-2)' }}>
          <span style={{ font: '600 11px/1 var(--font-sans)', color: 'var(--fg-3)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Baris Penawaran</span>
          {lines.map((l, i) => (
              ...
              {products.length > 0 && (
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (!p) { updateLine(i, { productId: null, productType: null }); return }
                    updateLine(i, {
                      productId: p.id,
                      productType: 'product',
                      description: p.name,
